...
#include <utils/shared_timed_recursive_mutex.h>
...
#else
        ALOGD( "Ingore priority_inheritance on windows os." );
#endif
    }
...
#ifdef _MSC_VER
            bool result = m_.try_lock_for( std::chrono::nanoseconds( timeout_ns ) );
            return result;
#else
...
#ifdef _MSC_VER
    shared_timed_recursive_mutex& std_mutex() {
        return m_;
    }
#else
    std::mutex& std_mutex() {
...
private:
#ifdef _MSC_VER
    shared_timed_recursive_mutex m_;
#else
    std::mutex m_;
#endif
...
#ifdef _MSC_VER
    std::unique_lock<shared_timed_recursive_mutex>& std_unique_lock() {
        return ul_;
    }
#else
...
private:
#ifdef _MSC_VER
    std::unique_lock<shared_timed_recursive_mutex> ul_;
    mutex& mutex_;
#else
